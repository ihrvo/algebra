...
SELECT 
    CONCAT(c.ime, ' ', c.prezime) AS "Ime i prezime",
    k.naslov AS "Naslov knjige"
FROM clanovi c
JOIN posudbe p ON c.id = p.clan_id
JOIN kopija ko ON p.kopija_id = ko.id
JOIN knjige k ON ko.knjiga_id = k.id;
...
SELECT z.naziv AS "Zanr", COUNT(k.id) AS "Broj knjiga"
FROM knjige k
JOIN zanrovi z ON k.zanr_id = z.id
GROUP BY z.naziv;
-- Ako želimo da se prikaže i broj dostupnih knjiga zajedno s kopijama
SELECT 
    z.naziv AS "Žanr",
    COUNT(k.id) AS "Broj dostupnih knjiga"
FROM zanrovi z
LEFT JOIN knjige k ON z.id = k.zanr_id
LEFT JOIN kopija ko ON k.id = ko.knjiga_id
WHERE ko.dostupna = TRUE
GROUP BY z.id;
