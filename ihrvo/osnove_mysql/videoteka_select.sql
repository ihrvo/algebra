SELECT p.datum_posudbe, p.datum_povrata, c.ime, f.naslov, m.tip, zanrovi.ime, cj.tip_filma, cj.cijena, cj.zakasnina_po_danu, CONCAT(DATEDIFF(p.datum_povrata, p.datum_posudbe)-1) AS 'Kašnjenje (dana)', 
CONCAT((DATEDIFF(p.datum_povrata, p.datum_posudbe)-1) * cj.zakasnina_po_danu + cj.cijena) AS 'Cijena'
    from posudba p
